        ...
        try {
            ...
            if (prods.length === 0) {
                return interaction.reply({ 
                    content: '❌ Nenhum produto cadastrado!\n\nUse `/painel` → botão **Criar Painel Vendas** → **➕ Criar Produto**', 
                    ephemeral: true 
                });
            }
            ...
            await interaction.reply({ 
                content: '📝 Selecione abaixo o produto que deseja editar:', 
                components: [new ActionRowBuilder().addComponents(sel)], 
                ephemeral: true 
            });
        } catch (err) {
            console.error('Erro /edita:', err);
...
async function showEditPanel(interaction, client, pid) {
    try {
        ...
        const r1 = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId(`edit_name_${pid}`).setLabel('✏️ Alterar Nome').setStyle(ButtonStyle.Primary),
        ...
        const r3 = new ActionRowBuilder().addComponents(
            ...
            new ButtonBuilder().setCustomId(`edit_delete_${pid}`).setLabel('🗑️ Excluir Produto').setStyle(ButtonStyle.Danger)
        );

        await interaction.update({ embeds: [embed], components: [r1, r2, r3], content: '' });
    } catch (err) {
        console.error('Erro showEditPanel:', err);
        await interaction.update({ content: `❌ Erro: ${err.message}`, components: [] });
    }
}
