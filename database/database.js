...
class Database {
    constructor() {
        if (process.env.NODE_ENV === 'production') {
            ...
            } catch { this.dbPath = path.join(__dirname, '..', 'bot.db'); }
        } else { this.dbPath = path.join(__dirname, 'bot.db'); }
        this.db = null;
    }
    ...
    async init() {
        try {
            ...
            return true;
        } catch (err) { console.error('❌ ERRO BANCO:', err.message); throw err; }
    }
    ...
    _createTables() {
        ...
        this.db.exec(`CREATE TABLE IF NOT EXISTS products(id INTEGER PRIMARY KEY AUTOINCREMENT,guild_id TEXT NOT NULL,panel_id TEXT DEFAULT '',name TEXT NOT NULL,description TEXT DEFAULT '',price REAL NOT NULL DEFAULT 0,stock INTEGER NOT NULL DEFAULT 0,auto_delivery TEXT,created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`);
        this.db.exec(`CREATE TABLE IF NOT EXISTS coupons(id INTEGER PRIMARY KEY AUTOINCREMENT,guild_id TEXT NOT NULL,name TEXT NOT NULL UNIQUE,discount REAL NOT NULL,expires_at DATETIME NOT NULL)`);
        this.db.exec(`CREATE TABLE IF NOT EXISTS sales(id INTEGER PRIMARY KEY AUTOINCREMENT,guild_id TEXT NOT NULL,user_id TEXT NOT NULL,username TEXT NOT NULL,product_id INTEGER NOT NULL,product_name TEXT NOT NULL,quantity INTEGER NOT NULL DEFAULT 1,total_price REAL NOT NULL,payment_method TEXT NOT NULL,status TEXT NOT NULL DEFAULT 'pending')`);
        this.db.exec(`CREATE TABLE IF NOT EXISTS reaction_emoji(id INTEGER PRIMARY KEY AUTOINCREMENT,guild_id TEXT NOT NULL UNIQUE,emoji TEXT NOT NULL)`);
        this.db.exec(`CREATE TABLE IF NOT EXISTS panels(id INTEGER PRIMARY KEY AUTOINCREMENT,guild_id TEXT NOT NULL,message_id TEXT NOT NULL,channel_id TEXT NOT NULL,title TEXT NOT NULL,description TEXT,banner_url TEXT)`);
    }
    ...
    addProduct(g,pid,name,desc,price,stock,auto=null){try{return this.db.prepare('INSERT INTO products(guild_id,panel_id,name,description,price,stock,auto_delivery)VALUES(?,?,?,?,?,?,?)').run(g,pid||'',name,desc||'',Number(price),Number(stock),auto).lastInsertRowid}catch(e){console.error('addProduct:',e);return null}}
    updateProduct(id,data){try{const f=Object.keys(data).map(k=>`${k}=?`).join(',');const v=Object.values(data);v.push(id);return this.db.prepare(`UPDATE products SET ${f} WHERE id=?`).run(...v).changes}catch(e){return 0}}
    ...
    updateStock(id,q){try{return this.db.prepare('UPDATE products SET stock=? WHERE id=?').run(q,id).changes}catch(e){return 0}}
