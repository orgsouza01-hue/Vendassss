...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        header{background:linear-gradient(135deg,#667eea,#764ba2);padding:30px;border-radius:15px;margin-bottom:30px;box-shadow:0 10px 30px rgba(102,126,234,.3)}
        header h1{font-size:2.5em;margin-bottom:10px}
        .stats-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:20px;margin-bottom:30px}
        .stat-card{background:rgba(255,255,255,.05);backdrop-filter:blur(10px);border:1px solid rgba(255,255,255,.1);padding:25px;border-radius:15px}
        .stat-card h3{font-size:.9em;opacity:.7;margin-bottom:10px;text-transform:uppercase}
        .stat-card .value{font-size:2em;font-weight:bold}
        .status-online{color:#00ff88}
        .section{background:rgba(255,255,255,.05);backdrop-filter:blur(10px);border:1px solid rgba(255,255,255,.1);padding:25px;border-radius:15px;margin-bottom:20px}
        .section h2{margin-bottom:20px}
        .item{background:rgba(255,255,255,.03);padding:15px;border-radius:10px;margin-bottom:10px;display:flex;justify-content:space-between}
        .item .price{color:#00ff88;font-weight:bold}
        footer{text-align:center;padding:30px;opacity:.6;margin-top:30px}
    </style>
</head>
<body>
    <div class="container">
        <header><h1>🤖 Bot de Vendas</h1><p>Painel Administrativo</p></header>
    ...
    <script>
        ...
        const u=Math.floor(s.uptime/1000);
        ...
        document.getElementById('products-list').innerHTML=p.length?p.map(x=>'<div class="item"><span><strong>'+x.name+'</strong></span><span class="price">R$ '+parseFloat(x.price).toFixed(2)+'</span></div>').join(''):'<p style="opacity:.6">Nenhum produto</p>';
        const c=await fetch('/api/coupons').then(r=>r.json());
        document.getElementById('coupons-list').innerHTML=c.length?c.map(x=>'<div class="item"><span><strong>'+x.name+'</strong></span><span class="price">'+x.discount+'% OFF</span></div>').join(''):'<p style="opacity:.6">Nenhum cupom</p>';
        const sl=await fetch('/api/sales').then(r=>r.json());
        document.getElementById('sales-list').innerHTML=sl.length?sl.slice(0,10).map(x=>'<div class="item"><span><strong>'+x.product_name+'</strong> - '+x.username+'</span><span class="price">R$ '+parseFloat(x.total_price).toFixed(2)+'</span></div>').join(''):'<p style="opacity:.6">Nenhuma venda</p>';
        }catch(e){}}load();setInterval(load,5000);
    </script>
</body>
</html>`;
